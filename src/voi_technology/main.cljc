(ns voi_technology.main
  "Kotodama WASM entrypoint for the Voi_technology clean-room actor (L4) — Clojure port.
  ...
  facts (`voi_technology.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "voi_technology")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "voitechn_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "voitechn_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"voi_technology.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "voi_technology-compat" :tier tier :entities entities} 200])
...
(defn start! [] :voi_technology-compat/ready)
